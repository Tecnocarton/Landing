...
/**
 * Registra el paso a WhatsApp con la cotización pre-llenada (F3.1).
 * Se mantiene separado de `contact_click` para poder medir aparte el
 * diferenciador (mensaje con specs) del clic al botón flotante genérico.
 * @param {Object} context
 * @param {string} context.origen - Dónde se hizo clic ('wizard_specs' | 'wizard_exito' | 'boxfinder')
 * @param {string} [context.producto] - Producto seleccionado en ese momento
 */
export const trackWhatsAppPrefilled = ({ origen = '', producto = '' } = {}) => {
  pushToDataLayer('whatsapp_prefilled', {
    origen,
    producto,
  });
};
...
/**
 * Registra la descarga/impresión del catálogo (F3.2).
 * @param {string} origen - Dónde se originó ('catalogo' = portada | 'catalogo_pie' = cierre)
 */
export const trackDescargaCatalogo = (origen = '') => {
  pushToDataLayer('descarga_catalogo', {
    origen,
  });
};
